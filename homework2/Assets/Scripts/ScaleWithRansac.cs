...
public class ScaleWithRansac : MonoBehaviour
{
    ...
    void Start()
    {
        Debug.Log("scale script started");
        Initialize();
    ...
    private List<Vector3> AlignPointsWithRansac(List<Vector3> sourcePoints, List<Vector3> targetPoints)
    {
        ...
        for (int i = 0; i < ransacIterations; i++)
        {
            ...
            float scale = ComputeScale(sampledSourcePoints, sampledTargetPoints, rotation, translation);

            // Count inliers
            int inliers = CountInliers(sourcePoints, targetPoints, rotation, translation, scale, threshold);
        ...
        // Apply the best transformation with scale
        return sourcePoints.Select(p => bestScale * (bestRotation * p) + bestTranslation).ToList();
    }

    private int CountInliers(List<Vector3> sourcePoints, List<Vector3> targetPoints, Quaternion rotation, Vector3 translation, float scale, float threshold)
    {
        int inliers = 0;
        for (int i = 0; i < sourcePoints.Count; i++)
        {
            Vector3 transformedPoint = scale * (rotation * sourcePoints[i]) + translation;
    ...
    private float ComputeScale(List<Vector3> P, List<Vector3> Q, Quaternion rotation, Vector3 translation)
    {
        float totalScale = 0;
        for (int i = 0; i < P.Count; i++)
        {
            Vector3 transformedPoint = rotation * P[i] + translation;
            float distanceP = Vector3.Distance(Vector3.zero, P[i]);
            float distanceQ = Vector3.Distance(Vector3.zero, Q[i]);
            totalScale += distanceQ / distanceP;
        }
        return totalScale / P.Count;
    }
